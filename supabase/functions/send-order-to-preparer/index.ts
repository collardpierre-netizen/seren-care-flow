...
serve(async (req) => {
  ...
  try {
    ...
    const baseUrl = 'https://www.serencare.be';
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 24px;">
    <div style="background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
      <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%); padding: 32px; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 24px;">📦 Nouvelle commande à préparer</h1>
      </div>
      <div style="padding: 32px;">
        ...
        <div style="background: #d1fae5; border-radius: 12px; padding: 20px; margin: 24px 0; border: 1px solid #6ee7b7;">
          <h3 style="margin: 0 0 12px; color: #065f46; font-size: 16px;">🔐 Lien sécurisé</h3>
          <p style="margin: 0; color: #047857; font-size: 14px;">Ce lien expire dans 7 jours.</p>
        </div>
        <div style="text-align: center; margin-top: 32px;">
          <a href="${preparationUrl}" style="display: inline-block; background: linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%); color: white; padding: 16px 40px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 16px;">Accéder à la commande</a>
        </div>
      </div>
      <div style="background: #f9fafb; padding: 24px; text-align: center; border-top: 1px solid #e5e7eb;">
        <p style="margin: 0; color: #9ca3af; font-size: 12px;">© 2026 SerenCare</p>
      </div>
    </div>
  </div>
</body>
    ...
    console.log('Email sent to:', data.preparerEmail);
