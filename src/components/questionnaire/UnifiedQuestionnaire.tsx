...
const UnifiedQuestionnaire: React.FC<UnifiedQuestionnaireProps> = ({
  ...
  // Implementation lives in `@/lib/questionnaireScoring` so it can be unit-tested
  // (notably for the profile-pre-fill case where `answers.mobility` is a French tag).
  const getRecommendedProducts = (): Product[] => scoreAndRecommendProducts(products, answers);
